...
#########################################
# TO DO: Compute sol of nonlin system
#matrix=mesh['matrix']
...
def create_data(num_data, p, force, kind):
    data = []
    if force == "sincos":
        ...
        for n in tqdm(range(num_data)):
            ...
            data.append([f_value, mesh[f'{kind}_coeff_fs'][n]])
...
data = create_data(NUM_DATA, p[idx_sol[0]], FORCE, KIND)
