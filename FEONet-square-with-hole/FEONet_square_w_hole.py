...
parser.add_argument("--model", type=str, default='Net2D', choices=['Net2D', 'FCNN'])
...
mesh=np.load("/home/haltmayermarc/FEONet_Stokes/FEONet-square-with-hole/data_ordered/P2x1_ne512_stokes.npz", allow_pickle=True)
...
if gparams['do_precond'] > 0:
    ...
    precond_file = "precond.npy"

    if os.path.exists(precond_file):
        print(f"Loading preconditioner from {precond_file}...")
        PRECOND = np.load(precond_file)
    else:
        print("Creating preconditioner...")
        PRECOND = spai(MATRIX, m)
        np.save(precond_file, PRECOND)
        print(f"Preconditioner saved to {precond_file}")

    cond_number = np.linalg.cond(MATRIX @ PRECOND)
    print("Done! Condition number:", cond_number)
...
if gparams['model'] == "Net2D":
    ...
    model_FEONet = MODEL(6, D_out, hidden_dims = [16, 32,128 ])
...
class Dataset(Dataset):
    def __init__(self, gparams, mesh, kind='train'):
        ...
        with open(f'/home/haltmayermarc/FEONet_Stokes/FEONet-square-with-hole/data_ordered/{kind}/' + self.pickle_file + '.pkl', 'rb') as f:
...
#log_path(PATH)
#log_gparams(gparams)
path = os.path.join(os.getcwd(), 'model_ordered')
...
for epoch in range(1, EPOCHS+1):
    ...
    for batch_idx, sample_batch in enumerate(trainloader):
        ...
        #train_u1_error += torch.sum(rel_L2_error(u_pred[:, IDX_SOL[0]], fenics_u1))
        #train_u2_error += torch.sum(rel_L2_error(u_pred[:, IDX_SOL[1]], fenics_u2))
        #train_p_error += torch.sum(rel_L2_error(u_pred[:, IDX_SOL[2]], fenics_p))
    ...
    if epoch%100==0:
        ...
        for batch_idx, sample_batch in enumerate(validateloader):
            with torch.no_grad():
                ...
                #_,u_pred = closure(model_FEONet, coeff_f, load_vec_f, MATRIX, RESOL_IN)
        ...
        #train_u1_errors.append(train_u1_error)
        #train_u2_errors.append(train_u2_error)
        #train_p_errors.append(train_p_error)
