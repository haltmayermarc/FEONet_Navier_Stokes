...
class Dataset(Dataset):
    def __init__(self, gparams, mesh, kind='train'):
        ...
        with open(f'/home/haltmayermarc/FEONet_Stokes/FEONet-square-with-hole_new/data_ordered/{BC}/{FORCE}/{kind}/' + self.pickle_file + '.pkl', 'rb') as f:
...
def weak_form(coeff_u, load_vec_f, matrix, precond):
    ...
    ## RHS
    RHS = load_vec_f.to(device)
    return LHS, RHS



def closure(model, coeff_f, value_f, load_vec_f, matrix, precond, resol_in):
    if gparams['model'] == "Net2D" or gparams['model'] == "UNetWithHead":
        if FORCE == "sincos":
            ...
            value_f = value_f.reshape(-1,2,resol_in,resol_in)
            pred_coeff_u = model(value_f)
    else:
...
for epoch in range(1, EPOCHS+1):
    ...
    for batch_idx, sample_batch in enumerate(trainloader):
        ...
        loss, u_pred = closure(model_FEONet, coeff_f, f_value, load_vec_f, MATRIX, PRECOND, RESOL_IN)
        loss.backward()
    ...
    if epoch%100==0:
        ...
        for batch_idx, sample_batch in enumerate(validateloader):
            with torch.no_grad():
                ...
                _,u_pred = closure(model_FEONet, coeff_f, f_value, load_vec_f, MATRIX, PRECOND, RESOL_IN)
                u_pred=u_pred.squeeze().detach().cpu()
